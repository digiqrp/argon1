...
# NB this throws issues with /dev/ram as ubuntu does not hae default
# for CONFIG_DEVMEM_PROTECTED set, its not enable in the default kernal now
# so it throws panic this means also that shutdown at the ,moment does not work
...
echo '	fanconfig = ["65=100", "60=55", "55=10","45=3"]' >> $powerbuttonscript
...
echo '		time.sleep(30)' >> $powerbuttonscript
echo >> $powerbuttonscript
# NB we do to start the powerbutton thread
# due to issues notes above
echo 'try:' >> $powerbuttonscript
...
echo "Description=Argon One Fan Service" >> $daemonfanservice
